...
- [x] **TestFlight pass, then submit for review.** ✅ **DONE 2026-07-05.** Internal testing group created, tester (`fduquedeestrada@icloud.com`, added as a Developer-role team member) completed a real TestFlight pass on-device 2026-07-05 and reported 4 bugs — all fixed (calendar-picker fix `cf67783`, i18n/EventKit logged as ROADMAP backlog). Bumped to `2.0.2` via `release.sh` (proper semver patch for the bug-fix build, not just a silent build-number bump), re-archived (`xcodebuild archive -destination "generic/platform=iOS"`), and exported+uploaded via CLI (`xcodebuild -exportArchive` with `destination=upload` in `exportOptions.plist` — much simpler than the Organizer GUI flow, no manual clicking needed and no rsync issue hit this time since Apple's rsync was already restored to PATH). Retest against `2.0.2 (1)` in TestFlight came back clean. **App Review notes: DONE 2026-07-05** — added to the App Review Information "Notes" field, covering (1) Google Sign-In uses standard OAuth, any Google account works, only the Calendar scope is requested; (2) the app needs a free Gemini API key from the user (link to `aistudio.google.com/apikey` included so reviewers aren't stuck); (3) cloud-storage imports (Drive/Dropbox/iCloud Drive/etc.) go through iOS's native file picker and require the provider app to be enabled as a Files "Location" first — a reviewer with none enabled won't see one appear as a source, which is expected, not a bug. "Sign-in required" was unchecked (no separate CEF account — OAuth covers it) and Contact Information (name/phone/email) filled in, both required before "Add for Review" would enable. Price tier set to Free. **Submitted for App Review 2026-07-05** — status "Waiting for Review," Apple's stated turnaround is up to 48 hours.
