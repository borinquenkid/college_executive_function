...
- [ ] **TestFlight pass, then submit for review.** Build is uploaded and compliance-cleared ("Ready to Submit") — still need to: create an internal testing group, add testers, do the actual TestFlight pass, then submit for App Review. Include App Review notes explaining how to test Google Sign-In — reviewers need a clear path through any login-gated flow, a common first-round rejection reason. Also note in the review notes that importing from Google Drive requires the separate Drive app to be installed on the test device (Drive access goes through iOS's native file picker now, not an in-app browser) — otherwise a reviewer without Drive installed may not find where "Drive" is and flag it as broken.
