...
## Phase 6b — Web Client & Agent-User Interaction (AG-UI) Protocol Integration ✅ DONE (6.5 completed 2026-07-24)
...
**Verified 2026-07-04:** 6.1–6.4 are done (server depends on `:composeApp`, `/web` Vite+React app exists with `useAgentStream.ts` + `App.tsx`, 890 lines; 6.2's SSE endpoint now emits real timestamps/runId, escaped JSON, and real Critic-Actor loop events). **6.5 completed 2026-07-24** — see below.
...
### 6.5 — Dynamic Agentic UI Views ✅ DONE (2026-07-24)

**Verification found the ROADMAP gap list itself was partly stale** — direct code inspection showed the tool-call itemization was already built (`App.tsx` already rendered `queryAllSources`/`actorPass`/`critiquePass` as distinct items with per-item status), and "Break it Down" wasn't a migration target — `POST /api/tasks/decompose`, which the web client called, didn't exist as a server route at all (verified against the full route list in `Application.kt`); it was dead code from the start.

* ✅ **Reasoning bubbles** — `useAgentStream.ts`'s `reasoning: string` (single concatenated paragraph) became `reasoningSteps: string[]`; each `REASONING_DELTA` pushes a new entry, and `App.tsx` renders each as its own bubble inside `.stream-reasoning-box` (new `.stream-reasoning-step` CSS rule) instead of one growing block.
* ✅ **Word-by-word streaming** — new `SseEventWriter.kt` (`server/.../SseEventWriter.kt`) extracts the SSE-frame-writing `emit()` closure previously duplicated inline per route, and adds `emitTextWordByWord()`: `TEXT_MESSAGE_START` → one `TEXT_MESSAGE_DELTA` per word+trailing-whitespace (25ms apart) → `TEXT_MESSAGE_END`, implementing the word-by-word contract SPEC.md §4.2 had documented but the server never actually did. `/api/agent/stream`'s chat route now uses this instead of a single `TEXT_MESSAGE_DELTA` with the full response.
* ✅ **Dynamic state rendering, via task decomposition** — built the previously-nonexistent feature for real rather than deferring it: `TaskDecompositionService` (already used by the desktop `TaskDecompositionDialog`, never wired into the shared `DependencyContainer`) is now a container-level service; a new `GET /api/events/{id}/decompose/stream` route (`WebTaskDecompositionHandler.kt`) streams its result as a `STATE_SNAPSHOT` event (`{"decomposedTasks": [...]}`, SPEC.md §4.2/4.1) instead of a REST round-trip. New `web/src/useDecomposeStream.ts` hook (mirrors `useSourceStream.ts`) replaces `App.tsx`'s dead `fetch('/api/tasks/decompose', ...)` call; "Break it Down" now actually works.

**Tests:** `AgentStreamTest.kt` updated (two tests that asserted the full response text as one contiguous substring now reconstruct it by concatenating ordered `TEXT_MESSAGE_DELTA` chunks — the old assertion shape doesn't hold under chunking) plus a new test asserting `TEXT_MESSAGE_START`/`_END` bracket multiple `TEXT_MESSAGE_DELTA`s. New `TaskDecompositionStreamTest.kt` (success path decodes the `STATE_SNAPSHOT` JSON including a title with an embedded quote/newline; event-not-found emits `ERROR`+`RUN_FINISHED`; a real-container test confirms 401 without a session). `web/src/test/setup.ts` gained a minimal `FakeEventSource` stub — jsdom has no native `EventSource`, and this phase was the first to exercise the app's SSE hooks (`useAgentStream`/`useSourceStream`/`useDecomposeStream`) from a test that actually opens one (the existing "Break it Down" a11y test in `App.test.tsx`).

**Verified:** `:server:test`, `:composeApp:jvmTest`, `:composeApp:checkQualityGate` (Quality Gate OK — `new_coverage: 83.8 ≥ 80`, `new_duplicated_lines_density: 0.0 ≤ 3`, `new_violations: 0`), `web`'s `tsc -b` / `eslint` / `vitest run` (10/10 passing) all green. Build Verification Protocol's four targets all pass. Manually verified live against a real running server (demo LTI login, `:server:run` + `npm run dev`): fetched `/api/agent/stream` and `/api/events/{id}/decompose/stream` directly and confirmed the exact event sequences (reasoning bubble → itemized tool calls → `TEXT_MESSAGE_START`/multiple `_DELTA`s/`_END` → `RUN_FINISHED`; decompose's not-found path → `ERROR`→`RUN_FINISHED`) against a real per-tenant `DependencyContainer`, not just mocks. The `STATE_SNAPSHOT` success path (a real decomposed-task list) is covered by the automated test, not manually walked — this dev environment has no configured Gemini API key to produce a real event to decompose against.
