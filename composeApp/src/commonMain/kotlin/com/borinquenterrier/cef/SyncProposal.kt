...
sealed interface SyncProposal {
    ...
    data class StudyBlockShift(
        ...
        override val description: String
            get() {
                val originalTime =
                    if (originalEvent is TimeEvent) "${originalEvent.startTime} - ${originalEvent.endTime}" else "All Day"
                val proposedTime =
                    if (proposedEvent is TimeEvent) "${proposedEvent.startTime} - ${proposedEvent.endTime}" else "All Day"
    ...
    data class DirectConflict(
        ...
        override val description: String
            get() {
                val localDesc =
                    if (localEvent is TimeEvent) "${localEvent.date} at ${localEvent.startTime}" else "${localEvent.date} (All Day)"
                val remoteDesc =
                    if (remoteEvent is TimeEvent) "${remoteEvent.date} at ${remoteEvent.startTime}" else "${remoteEvent.date} (All Day)"
