...
actual class PdfReader(private val logger: Logger? = null) {
    actual suspend fun readSource(path: String): List<SourceFragment> =
        withContext(Dispatchers.Default) {
            try {
                val url = NSURL(string = path)
                    ?: return@withContext listOf(SourceFragment("Invalid PDF path"))
                val document = PDFDocument(uRL = url)
                    ?: return@withContext listOf(SourceFragment("Could not load PDF document"))
