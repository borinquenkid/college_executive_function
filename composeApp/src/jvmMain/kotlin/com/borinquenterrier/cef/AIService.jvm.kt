...
actual class AIService {
    actual suspend fun generateChatResponse(prompt: String): String {
        delay(1000)
        return "This is a dummy chat response to: '$prompt'"
    }
    ...
    actual suspend fun generateCalendarEvents(prompt: String): List<CalendarEvent> {
        ...
        return listOf(
            CalendarEvent("Dummy Event 1", now, now.plus(1, DateTimeUnit.HOUR)),
            CalendarEvent("Dummy Event 2", now.plus(2, DateTimeUnit.HOUR), now.plus(3, DateTimeUnit.HOUR))
        )
    }
}
