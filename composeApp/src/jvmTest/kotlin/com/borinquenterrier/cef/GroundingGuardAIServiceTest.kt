...
class GroundingGuardAIServiceTest : FunSpec({
    ...
    test("generateStudyPlan drops events whose year never appears in the source — closes the gap that produced academic_calendar.ics") {
        ...
        // The source text contains no year at all — extractSourceYears returns an empty set,
        // which is itself the strongest possible signal that nothing the model returns can be
        // grounded. filterToSourceYears intentionally no-ops on an empty source-year set (to
        // avoid punishing legitimately yearless documents), so this fixture alone wouldn't have
        // caught the original bug — but it does prove the guard is wired into this path at all,
        // which generateStudyPlan previously was not.
        val result = guard.generateStudyPlan(syllabusText)
        result.shouldHaveSize(3)

        // Now show the guard actually bites once the source gives it something to check against.
        coEvery { delegate.generateStudyPlan(any(), any(), any()) } returns listOf(
            dayEvent(2024, 10, 14, "Midterm Exam"),       // grounded — 2024 is in the source
            dayEvent(2099, 9, 1, "Class Meeting")          // confabulated — 2099 appears nowhere
        )
