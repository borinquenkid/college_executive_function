...
3. Rotate: `GOOGLE_CLIENT_ID`/`GOOGLE_CLIENT_SECRET`, `CEF_GEMINI_API_KEY`, `SONAR_TOKEN`,
   ...
   `~/zed/second_brain/ops/runbooks/secret-rotation-runbook.md`** (ROADMAP.md Phase 11 Task 10) —
   not duplicated here since that doc is the maintained source of truth, this bullet is just the
   "what to rotate" list.
...
4. ✅ **DONE 2026-07-09** — **Dependabot version updates.** Added `.github/dependabot.yml` for
   both `gradle` (directory `/`, walks the whole multi-module tree — root, `androidApp/`,
   `composeApp/`, `iosApp/`, `server/`, `shared/`) and `npm` (directory `/web`), weekly schedule,
